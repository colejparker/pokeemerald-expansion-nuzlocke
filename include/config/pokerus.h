...
#define P_POKERUS_ENABLED                FALSE      // If FALSE, Pokérus will have no effect, won't be shown and won't be aquired in any way but save data won't be affected. Disabled in this hack: the underlying per-mon byte (MON_DATA_POKERUS) is repurposed as the permanent-faint marker for the nuzlocke tracker instead — see MarkMonPermanentlyFainted/IsMonPermanentlyFainted in pokerus.c
