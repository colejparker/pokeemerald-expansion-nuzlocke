...
- Pokemon are randomized (built with `Zetraphes/pokeemerald-expansion@tertu-randomizer` as a reference; deviated from it where this codebase already had better-suited infrastructure — see `include/randomizer.h`/`src/randomizer.c` for the core engine)
    ...
    - [x] Abilities are randomized from a whitelisted list of abilities (`RandomizeAbility` in `src/randomizer.c`, hooked into the single canonical lookup `GetSpeciesAbility` in `src/pokemon.c` — every one of its ~30 call sites across battle AI, DexNav, the party menu Ability Capsule/Patch logic, Trace/Skill Swap checks, etc. gets the randomized ability for free with no call-site changes, since they all route through this one function. Broad whitelist, matching your call — every ability is eligible except a short list of form-change-locked ones (Zen Mode, Stance Change, Shields Down, Schooling, Disguise, Battle Bond, Power Construct, Gulp Missile, Hunger Switch) that are tied to specific species' form tables and would risk misbehaving on an unrelated species rather than just being "weird." `originalAbility == ABILITY_NONE` is always passed through unchanged, preserving "does this species even have a 2nd/3rd ability slot" checks used throughout that code)
        ...
        - [x] Shedinja always has Wonder Guard [ ] but its type is randomized (not yet done — ability lock is in via a species-id exception in `RandomizeAbility`; the type-randomization half is a separate, narrower feature not started)
        - [x] Ditto always has Imposter [ ] but its stat spread is entirely randomized (not yet done — same ability-lock mechanism as Shedinja; the stat-spread-randomization half is unrelated to this species/ability randomizer and not started)
...
- [x] Items are randomized (`RandomizeFieldItem` in `src/randomizer.c`, hooked into `src/item_ball.c` `GetItemBallIdFromTemplate` for visible item balls and `src/field_control_avatar.c`'s `BG_EVENT_HIDDEN_ITEM` case for hidden items — the two choke points before an item id reaches `additem`/`finditem`. Seeded from map + the object event's/hidden item's local id, so a given pickup always resolves to the same item on a save)
    - [x] Any item given to the player is randomized from a list, with the exception of HM's and the key items needed for story progression (excludes the whole Key Items pocket, not just specific story items — randomizing any key item risks breaking progression or removing a QOL item, so the exclusion is blanket rather than curated)
    - [ ] All berry trees are replaced by items a player can pick-up (unrelated feature, not started)
    - [x] The TM's a player can get are randomized, HM's are kept the same (a found TM always randomizes to another TM slot; HMs are excluded from randomization entirely and always give the original HM)
...
| Moveset randomizer (21 moves, STAB/non-STAB/status split, power curve to Lv63) | Would replace the per-species level-up learnset arrays |
