...
- [x] Starter Pokemon always start with 5 perfect IV's (`src/battle_setup.c` `CB2_GiveStarter` — builds the starter mon itself instead of delegating to `ScriptGiveMon`, then calls `SetBoxMonPerfectIVs(&mon.box, 5)` before handing it to the player. Deliberately done at the give-site rather than via the species' `.perfectIVCount` field, since that field is shared with wild/gift encounters of the same species and would leak the guarantee outside the starter pick)
